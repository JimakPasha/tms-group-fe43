...
function collectGenres(films: IFilms[]): any{
    let genresArr = films.reduce((genres: string[], film: IFilms) =>{
        return [...genres, ...film.genre];
    },[])
...
console.log(collectGenres(films));


// Собрать в массив всех актеров всех фильмов (без повторения)


function collectActors(films: IFilms[]): any{
    let actorsArr = films.reduce((actors: string[], film: IFilms) =>{
        return [...actors, ...film.actors];
    },[])

   const uniqueActors: string[] = [];

   actorsArr.forEach((actor: string) =>{
...
console.log(collectActors(films));


// Отсортировать фильмы по рейтингу по убыванию


function sortRatings(films: IFilms[]): IFilms[]{
   return films.sort((a, b) => b.imdbRating - a.imdbRating);
}

console.log(sortRatings(films));
...
const newObj: newArr[] = films.map(film => ({
    id: film.id,
    title: film.title,
...
function filterFilmsByYear(films: IFilms[], year: number): IFilms[]{
    const filteredFilms = films.filter(film => film.year === year);
    return filteredFilms;
}


console.log(filterFilmsByYear(films, 2009));


// Создать функцию, которая бы принимала массив фильмов и строку. А результатом этой функции должен быть новый отфильтрованный массив, с фильмами, где строка входит в название фильма.


function filterFilmsByPhrase(films: IFilms[], phrase: string): IFilms[]{
    const filteredFilms = films.filter(film => film.title.toLowerCase().includes(phrase.toLowerCase(), 0));
    return filteredFilms;
}


console.log(filterFilmsByPhrase(films, 'harry'));


// Создать функцию, которая бы принимала массив фильмов и строку. А результатом этой функции должен быть отфильтрованный массив, с фильмами где строка входит в название фильма или в его сюжет.


function filterFilmsByPlot(films: IFilms[], phrase: string): IFilms[]{
    const filteredFilms = films.filter(film => film.title.toLowerCase().includes(phrase.toLowerCase(), 0) || film.plot.toLowerCase().includes(phrase.toLowerCase(), 0));
    return filteredFilms;
}


console.log(filterFilmsByPlot(films, 'luke'));
...
function filterByParams<T>(films: IFilms[], key: keyof IFilms, str: string):IFilms[]{
    return films.filter(item => item[key] == str)
}

console.log(filterByParams<string | number>(films, 'title',  "Harry Potter and the Sorcerer's Stone"));
...
const ratingSort = movies
  .sort((rate1, rate2) => rate2.imdbRating - rate1.imdbRating)
...
let arrFindYear: IMovies[];
function getYear(movies: IMovies[], num: number) {
...
console.log(getYear(movies, 2001));
...
let arrFindTitle: IMovies[];
function getTitle(movies: IMovies[], str: string) {
...
// А результатом этой функции должен быть отфильтрованный массив, с фильмами где строка входит в название фильма или в его сюжет.
let arrFindTitlePlot: IMovies[];
function getTitlePlot(movies: IMovies[], str: string) {
  arrFindTitlePlot = movies.filter((movie) => {
    if (
      movie.title.toLowerCase().includes(str.toLowerCase()) ||
      movie.plot.toLowerCase().includes(str.toLowerCase())
    ) {
      return movie;
    }
  });
...
// 8. Создать функцию, которая бы принимала 3 параметра: 1)массив фильмов , 2) строка(название поля, например 'title')
...
console.log(getFindArr<string | number>(movies, "title", "Star Wars"));

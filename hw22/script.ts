...
const genres: string[] = films.reduce((acc: string[], film) => {
...
const allActors: string[] = films.reduce((acc: string[], actor) => {
    actor.actors.forEach((actors) => {
        if (!acc.includes(actors)) {
            acc.push(actors);
        }
    });
    return acc;
...
const sortedFilms: IFilms[] = films.sort((a, b): any => {
...
type FilmNotAll = Pick<IFilms, 'id' | 'title' | 'released' | 'plot'>;
const notAllKeys: FilmNotAll[] = films.map(movie => ({
    id: movie.id,
    title: movie.title,
...
function filterFilmByYear(films: IFilms[], year: number) {
    return films.filter(movie => movie.year === year);
};

console.log(filterFilmByYear(films, 2021));
...
function filterMoviesByTitle(films: IFilms[], title: string): IFilms[] {
    return films.filter((movie) => {
        return movie.title.toLowerCase().includes(title.toLowerCase());
    });
};

console.log(filterMoviesByTitle(films, 'harry potter'));
...
function filterFilmByTitleOrPlot(films: IFilms[], b: string) {
    return films.filter(film => {
        const title = film.title.includes(b);
        const plot = film.plot.includes(b);
        return title || plot;
    });
};

console.log(filterFilmByTitleOrPlot(films, 'wizardry'));
